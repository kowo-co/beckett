...
| `beckett task steer <run> "…"` | Send a mid-flight instruction to the running worker; on a `parked` or `awaiting_input` run this resumes it instead of being refused. |
| `beckett task resume <run> [--note "…"] [--answer "…"]` | Clear a parked run's blocker and re-staff the stage it parked from, or answer an `awaiting_input` run's open question. |
| `beckett task courier <run> [--pr-url <url>]` | Mark a run `done` after its work was published by hand (or its PR merged) outside the machinery. |
