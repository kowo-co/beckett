...
- `bun run test`: **green** (3840 pass, 2 skip, 2 fail — both failures are pre-existing on
  `44be996` before this PR touched anything: `dispatch/spawn.test.ts`'s betterwright-binary-cwd
  test and `drivers/cursor.test.ts`'s credential-preflight test; confirmed via `git stash` against
  the unmodified tree).
