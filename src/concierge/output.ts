 ...
 *
 * The already-parsed two-field shape must stay accepted here: real sessions return the result of
 * {@link parseDiscordTurnOutput}, which has no `voice_check`. Re-running the raw three-field parse
 * on it would coerce every real answer to a silent pass.
 */
export function coerceDiscordTurnOutput(value: unknown): DiscordTurnOutput {
  ...
  if (value && typeof value === "object" && !Array.isArray(value)) {
    const output = value as Record<string, unknown>;
    const keys = Object.keys(output);
    if (keys.length === 2 && keys.includes("decision") && keys.includes("message")) {
      if (output.decision === "send" && typeof output.message === "string" && output.message.trim()) {
        return { decision: "send", message: output.message.trim() };
      }
      if (output.decision === "pass" && output.message === null) return { decision: "pass", message: null };
