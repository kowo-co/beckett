...
import type { ParsedChecklist } from "./spec-file.ts";
 ...
 */
export function runBody(run: Run, spec?: ParsedChecklist): string {
  const goal = `## Goal\n${run.prompt.trim()}`;
 ...
 */
export function runAsTicket(run: Run, spec?: ParsedChecklist): Ticket {
