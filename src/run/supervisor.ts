...
export class RunSupervisor {
  ...
  private readSpec(run: Run): ParsedChecklist | undefined {
  ...
  private async doSpawn(run: Run, stage: RunStage, reservation: symbol): Promise<void> {
    ...
    try {
      ...
      this.noteSpawnFailure(run, `could not provision the project repo at \`${repoRoot}\`: ${(err as Error).message}`);
      return;
    }
    ...
    try {
      ...
      this.noteSpawnFailure(run, `could not allocate a worktree under \`${repoRoot}\`: ${(err as Error).message}`);
      return;
    }
    ...
    if (!existsSync(specPath)) {
      try {
        writeFileSync(specPath, renderSpecScaffold({ ...run, workspace }), "utf8");
      } catch (err) {
  ...
  private async finishImplement(
    ...
    this.store.update(run.id, { state: "reviewing" });
  ...
  private async finishReview(
    ...
    this.store.update(run.id, { reviewCycles: cycles });
    ...
    this.store.update(run.id, { state: "implementing" });
    const next = this.store.get(run.id);
  ...
  private async publishRun(run: Run, summary: string): Promise<void> {
    this.store.update(run.id, { state: "publishing" });
   ...
   */
  private noteSpawnFailure(run: Run, reason: string): void {
    this.store.update(run.id, { error: reason });
  ...
  private async park(run: Run, reason: string): Promise<void> {
    this.store.update(run.id, { state: "parked", error: reason });
